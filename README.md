...
The installer ends with an interactive wizard that asks for an
admin password (8 chars minimum) and prints the paper recovery
code once. **Write it down before you continue** — that's the only
time it's shown.
...
98 tests. On a Windows dev host 1 file is skipped at collection time
(the UI-formatter tests, which need PyQt5) and 4–5 POSIX-only tests
skip at runtime. All 98 should run and pass inside the Rocky VM.
...
Defaults and the hardened systemd unit are in `src/config/` and
`src/systemd/`. Install/uninstall scripts live in `src/scripts/`.
